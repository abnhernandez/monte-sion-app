...
import { getCurrentUserForHeader, logoutAction } from "@/lib/auth-actions"
...
export function BarradeNavegacion() {
  ...
  useEffect(() => {
    startTransition(async () => {
      ...
      if (user) {
        setUsername(displayNameFrom(user.username || user.email) || "Usuario")
              ...
              <>
                ...
                {avatarMenuOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border z-50 animate-fade-in flex flex-col overflow-hidden">
                    ...
                    <button className="text-left px-4 py-3 hover:bg-gray-100 font-semibold text-red-600 text-base border-t border-gray-100" onClick={() => { startTransition(() => logoutAction()); setAvatarMenuOpen(false); }}>Salir</button>
                  </div>
                )}
              </>
            ) : (
